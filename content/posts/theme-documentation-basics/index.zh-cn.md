...
由于 Hugo 提供的便利性, 如静态博客，加载速度快等特性，[Hugo](https://gohugo.io/) 本身是这个主题唯一的依赖.

可以选择Netlify作为博客服务器供应商， 提供免费的静态博客托管服务和免费二级域名。但国内不建议使用Netlify CMS，因为基本不能稳定访问。

不额外增加图床，而将图片资源同步放在博客目录。
...
在[Hugo Release](https://github.com/gohugoio/hugo/releases) 查看 Hugo的最新版本，

用最新版本号替代下面代码里面的0.96.0，你也可以直接复制下面的代码，进行下载和安装。
```Bash
$ wget https://github.com/gohugoio/hugo/releases/download/v0.96.0/hugo_0.96.0_Linux-64bit.deb # 使用 wget 下载 hugo
$ wget https://github.com/gohugoio/hugo/releases/download/v0.96.0/hugo_extended_0.96.0_Linux-64bit.deb # 使用 wget 下载 hugo
$ sudo dpkg -i hugo_0.96.0_Linux-64bit.deb hugo_extended_0.96.0_Linux-64bit.deb
```
...
建议在项目根目录下新建config.toml并拷贝下面的基本配置进行扩展配置，而不是直接cp主题模板的完全config.toml，因为hugo server会出现问题。

基本以下是 LoveIt 主题的基本配置:
...
# 或者参考主题模板，可以新建./contend/posts/下新建一个文件夹，index-en.md为英文，index-zh-ch.md为中文，再放上一些资源文件
hugo new posts/first_post/index-zh-ch.md
